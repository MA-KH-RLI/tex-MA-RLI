...
Urbane Netzgebiete können derzeit nicht durch \gls{DINGO} abgebildet werden und werden deshalb innerhalb dieser Arbeit nicht betrachtet. \medskip
...
Um mit dieser Arbeit eine Ergänzung zu \textit{E-Mobility Study} \cite{Schachler} zu liefern, werden die Ergebnisse des Clusterings aus \textit{E-Mobility Study} übernommen und von den \num{15} identifizierten repräsentativen Netzgebieten eine Teilmenge untersucht.
So werden jeweils zwei Netzgebiete der Kategorien \gls{PV}-, Wind- und Last-dominiert ausgewählt, die möglichst viele Netzgebiete repräsentieren.
...
Weiterhin findet sich in \autoref{fig:map_representatives} eine Karte der repräsentierten Netzgebiete eingeteilt in die Kategorien \gls{PV}-, Wind- und Last-dominiert.
...
Mit Hilfe des im Rahmen dieser Arbeit mitentwickelten Software Tools \gls{SIMBEV} können die Fahrtprofile für eine beliebige Anzahl an Fahrzeugen verschiedener Fahrzeugklassen erstellt werden.
...
Dies bedeutet, dass die Gesamtanzahl der Fahrzeuge je Fahrzeugklasse und Szenario entsprechend des heutigen Bestandes anteilig verteilt wird.
...
In der Realität wird es vorkommen, dass Fahrzeuge außerhalb ihres während der Regionalisierung zugewiesenen geographischen Gebietes geladen werden.
...
Es ist vorstellbar, dass in einzelnen Fällen auch größere Verschiebungen entstehen, wenn z.B. innerhalb eines geographischen Gebietes ein großer Ladepark aufgebaut wird oder ein großes Unternehmen ansässig ist, welches als Anlaufpunkt für eine Vielzahl von Fahrzeugen aus anderen Gebieten dient. 
...
Bei der Ladeinfrastruktur auf Firmenparkplätzen werden sehr große Flächen identifiziert, die in der Regel nicht nur einem Unternehmen zugewiesen werden können.
Da hierdurch unter Umständen nur wenige Anschlusspunkte generiert werden, werden ergänzend \SI{50}{\percent} der möglichen öffentlichen Normalladeinfrastruktur zusätzlich für Ladeinfrastruktur auf Firmenparkplätzen genutzt.
...
Der Anschluss erfolgt hierbei bis zu einer Anschlussleistung von \SI{0.3}{\mva} in der Niederspannung und bei höheren Anschlussleistungen direkt in der Mittelspannung.
...
Erfolgt der Anschluss einer Ladestation auf der \gls{MS}-Ebene, dann wird diese Ladestation wie ein eigener \gls{NS}-Strang behandelt und die einzelnen Ladepunkte dementsprechend in die Gruppen eingeteilt.
...
Der Ladevorgang eines Fahrzeuges findet hierbei immer innerhalb der Zeitpunkte der Standzeit statt, welche die geringste Residuallast innerhalb des \gls{MS}-Netzes aufweisen, wodurch eine Glättung der Residuallast erreicht werden soll.
...
Anschließend wird die Last bzw. die Einspeisung in Schritten von \SI{5}{\percent} innerhalb der ermittelten Zeitschritte reduziert.
...
Dabei werden zuerst die einzelnen \gls{NS}-Netze eines \gls{MS}-Netzgebietes einzeln betrachtet, um dort extreme Belastungssituationen abzufangen, die eine Konvergenz des Gesamtsystems verhindern können.
