...
Um die Anzahl an \glspl{EPKW} je Netzgebiet zu bestimmen, muss der Gesamtbestand an \glspl{EPKW} je Szenario (s. \autoref{tab:SzenarienRampUp} und \autoref{tab:CarSplit}) regionalisiert werden.
...
Diese entspricht dem Verhältnis von \glspl{EPKW} im Netzgebiet und der Anzahl möglicher Ladepunkte je \UC multipliziert mit einem Faktor von \num{5}.
...
Wenn nicht, dann fallen die zugeordneten Ladevorgänge innerhalb eines angrenzenden Netzgebietes an und sind somit nicht Teil der abschließenden Auswertungen.
...
Es wird darauf geachtet, dass die Zuweisung der Gruppen nicht nur innerhalb eines Netzgebietes leistungsbezogen möglichst homogen erfolgt, sondern detailliert bis in die einzelnen Stränge der \gls{NS}-Ebene.
